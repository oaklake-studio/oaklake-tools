...
**Do NOT use when:**
- The demo needs a **real third-party SSO login** on camera (Google/Okta/…). Hands-off automation is blocked by modern browsers — use hybrid capture: run headed, drive to the provider's screen, then poll only the URL (never screenshot credentials) while the **user** signs in; delete the throwaway profile after.
...
POSIX shell required (macOS/Linux; on Windows run this under WSL or git-bash). Keep Playwright in a **dedicated persistent venv** outside any project — dependency managers (`uv sync`, `poetry install`) prune anything not in their lockfile, which is why an in-project install "disappears."
